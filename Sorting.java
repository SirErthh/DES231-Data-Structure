public class Sorting {
	...
	static void SelectionSort(int[] A) { // Ex 1a Complete the method SelectionSort
		for (int i = 0; i < A.length - 1; i++) { //ทำจากตัวเเรกสุดถึงตัวก่อนสุดท้าย
			int minIndex = i; // Index of smallest remaining value.
			minIndex = findIndexSmallest(A, i, A.length-1);//หาmin indexถึงตัวสุดท้าย methodนี้มี<=
			// Ex1.Complete this method. Note that the method swap is provided.
			if (minIndex!=i){ //indexตรงกับ iterationอยู่ที่เดิมเเสดงว่ามันไม่สลับ
				swap(A, i, minIndex);
				printArray(A);
			}
		}
	...
	static void InsertionSort(int[] A) { // Ex 1b Complete the method InsertionSort
		for (int i = 1; i < A.length; i++) { //เช็คจากตัวที่ 2 ไปถึงตัวสุดท้าย
			 // assignค่าtemp ของเเต่ละ iteration
			for (int j = i - 1; j >= 0 && A[j] > A[i]; j--) { // jเป็นตัวที่1 อยู่หน้า i , J>=0 คือมันไม่เอาout of index ,A[j]>temp compareมากกว่าswap น้อยกว่าไปอยู่หน้า
				// Ex2.Complete this method. Note the method swap is provided.
				swap(A, j, j+1); //swapที่ตัวcompare กับ ตัวเปรัยบเทียบ
				printArray(A);
			}
		}
	}
	...
	private static void merge(int[] A, int[] leftHalf, int[] rightHalf) {
		...
		int i = 0, j = 0, indexcom = 0;

		while (i < leftSize && j < rightSize) {
			if (leftHalf[i] < rightHalf[j]) { //เช็คว่าขวา มากกว่าซ้าย ตัวน้อยกว่าก็จะเป็นตัวเริ่มของcombine
				A[indexcom] = leftHalf[i]; //assign left half value ใน combine
				i++; //เลื่อน i ของ leftside ไป1
				indexcom++; //เพิ่มindexของcombine ไป1ช่อง
			} else {
				A[indexcom] = rightHalf[j];
				j++; //เลื่อน i ของ rightside ไป1
				indexcom++; //เพิ่มindexของcombine ไป1ช่อง
			}
		}
		// Copy any remaining elements from the leftHalf array, if any.
		while (i < leftSize) {
			A[indexcom] = leftHalf[i];
			i++;
			indexcom++;
		}
		// Copy any remaining elements from the leftHalf array, if any.
